...
#define GRID_EXTRAX1 6
#define GRID_EXTRAX2 12
#define GRID_EXTRAX3 6
#define GRID_EXTRAY1 3
#define GRID_EXTRAY2 6
#define GRID_EXTRAY3 3
...
void notebook::bitmap_draw_cellgroup_bracket(boxnode * us, int level, int globx, int globy, bool selected)
{
...
//boxnode_print(bfrom_ptr(NULL), bfrom_node(us), 0);
    ...
    while (last->header.type == BNTYPE_CELLGROUP)
    {
        if (last->extra1 & BNFLAG_OPEN)
        {
//std::cout << " reading from last !!!!!!!!!!!!!!!!!" << std::endl;
            lastoffy +=  last->array[last->len-1].offy;
            last = bto_node(last->array[last->len-1].child);
        }
        else
...
//std::cout << " reading from first !!!!!!!!!!!!!!!!!" << std::endl;
    ...
    firstoffy += first->array[0].offy; first = bto_node(first->array[0].child);
    int firstsizey = first->header.sizey;
    lastoffy += last->array[0].offy; last = bto_node(last->array[0].child);
    int lastsizey = last->header.sizey;
...
//std::cout << " lastsizey: " << lastsizey << std::endl;
    ...
    int32_t cellbracket_w = glb_dingbat.get_char_sizex(DINGBAT_CELLGEN, zoomint);
    int32_t cellbracket_h = glb_dingbat.get_char_sizey(DINGBAT_CELLGEN, zoomint);
...
{
//std::cout << "rootbox::draw_pre" << std::endl;

    if (cursor_b > childcells.size())
    {
        assert(cursor_a < childcells.size());
        childcells[cursor_a].cbox->draw_pre(boxdrawarg(da.nb, da.globx + childcells[cursor_a].offx, da.globy + childcells[cursor_a].offy, 0, da.T));
        return;
    }
    else if (cursor_b == cursor_a)
    {
        int32_t y = cursor_a < childcells.size() ? childcells[cursor_a].offy : sizey;
        drawtline(0, y, glb_image.sizex, y, da.nb->zoomint*0.03, da.nb->cCursor, da.T);
        return;
    }
    else
    {
        // TODO: draw cell selection
        return;
    }
}
...
{
//std::cout << "rootbox::draw_main " << da.tostring() << std::endl;

    if (cursor_b > childcells.size())
    {
        for (int32_t i = 0; i < childcells.size(); i++)
        {
            childcells[i].cbox->draw_main(boxdrawarg(da, childcells[i].offx, childcells[i].offy, cursor_a == i ? 0 : DFLAG_IGNORESEL));
        }        
    }
    else
    {
...
{
//std::cout << "rootbox::draw_post " << da.tostring() << std::endl;

    if (cursor_b > childcells.size())
    {
        assert(cursor_a < childcells.size());
        childcells[cursor_a].cbox->draw_post(boxdrawarg(da, childcells[cursor_a].offx, childcells[cursor_a].offy));
        return;
    }
    else if (cursor_b == cursor_a)
    {
...
{
//std::cout << "cellbox::draw_pre" << std::endl;
...
{
std::cout << "subscriptbox::draw_pre" << std::endl;
...
{
//std::cout << "subsuperscriptbox::draw_pre" << std::endl;
...
graphics3dbox::graphics3dbox()
        ...
        fxn_z(emake_node(gs.sym_sFunction.copy(), emake_node(gs.sym_sTimes.copy(), emake_node(gs.sym_sSlot.copy(), emake_cint(3)), emake_cint(2))))
    {
        ...
        inputline = "Compile[{x:Real, y:Real, z:Real}, x^2+y^2+z^2-0.9]";
        std::vector<uex> v;
        syntax_report sr;
...
//std::cout << "rowbox::draw_main " << da.tostring() << std::endl;
    ...
    if (da.dflags & DFLAG_SCOLOR)
    {
        ...
        {
            bsymer B;
            B.add_scolor(child.data() + 0, 0, child.size() - 1);
...
void notebook::key_pageup()
{
    ...
    offy = std::min(offy, 0);
    offy = std::max(offy, -root->header.sizey + int(glb_image.sizey/2));
}
...
void notebook::fitcursor()
{
    ...
    int32_t pady = std::max(20, glb_image.sizey/8);
    if (offy < pady - y)
    {
        offy = pady - y;
    }
    ...
    offy = std::min(offy, 0);
    offy = std::max(offy, -root->header.sizey - glb_image.sizey/2);
...
//std::cout << "measuring notebook:" << std::endl;
//myroot->print(0,0,0);
...
//printf("*******mesauring*******:\n");
    measure();
//printf("*******done mesauring*******:\n");

//std::cout << "drawing notebook:" << std::endl;
//myroot->print(0,0,0);
...
//uint64_t b = GetMS();
//puts("calling _draw_bitmap");
//printf("before: %d, %d\n", offx, offy);
//printf(" after: %d, %d\n", offx, offy);

//    _draw_cursor_selection(offx, offy);
    ...
    T.cos_theta = 1.0;
    T.sin_theta = 0.0;
//    _draw_bitmap(root, 0, offx, offy, 0, &T);

//printf("draw_pre\n");
    myroot->draw_pre(boxdrawarg(this, offx, offy, 0, &T));
//printf("draw_main\n");
    myroot->draw_main(boxdrawarg(this, offx, offy, 0, &T));
//printf("draw_post\n");
    myroot->draw_post(boxdrawarg(this, offx, offy, 0, &T));
//printf("draw done\n");

//    _draw_post(offx, offy);

//uint64_t c = GetMS();
//std::cout << "measure: " << b-a << "ms  draw: " << c-b << "ms" << std::endl;
}
